...
function factory(
  ...
  if (root && !root.classList.contains('fui-combobox')) {
    throw new Error('[Faisceau UI] Combobox enhance mode need a root with the `fui-combobox` class')
  }
  ...
  const bindItem = (element: HTMLElement, item: FuiItem): void => {
    ...
    if (indicator) disposers.push(zag.bind(indicator, (api) => api.getItemIndicatorProps({ item })))
